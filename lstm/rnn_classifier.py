    ...
    def forward(self, x, h_0=None):
        ...
        for step in range(seq_len):
            ...
            for i, l in enumerate(self.rnn):
                hidden[i, :, :] = l.forward(x, self.hiddens[-1][i])
                x = hidden[i, :, :]
            self.hiddens.append(hidden.copy())

        return self.output_layer.forward(x) # Get the outputs from the last time step using the linear layer and return it
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for step in reversed(range(seq_len)):
            for i in reversed(range(self.num_layers)):
                # Get h_prev_l either from hiddens or x depending on the layer
                if i != 0:
                    h_prev_l = self.hiddens[step+1][i-1]
                else:
                    h_prev_l = self.x[:, step, :]
                # Get dx, & Update dh
                dx, dh[i] = self.rnn[i].backward(dh[i], self.hiddens[step+1][i], h_prev_l, self.hiddens[step][i])
                # Add dx if you aren't at the first layer
                if i != 0:
                    dh[i-1] += dx
        
        return dh / batch_size # Normalize dh by batch_size since initial hidden states are also treated
